...
function nictitate_lite_ii_toolkit_print_single_post_author() {
    global $post;
...
function nictitate_lite_ii_toolkit_build_query( $query_args = array() ) {
...
function nictitate_lite_ii_toolkit_get_allowed_tags() {
    if( function_exists( 'nictitate_lite_ii_get_allowed_tags' ) ){
        return nictitate_lite_ii_get_allowed_tags();
    }else{
        return wp_kses_allowed_html( 'post' );
    }
}

function nictitate_lite_ii_toolkit_get_client_ip() {
    $ip = false;
...
function nictitate_lite_ii_toolkit_get_likes( $post_id = 0 ) {
    $key = 'nictitate_likes';
    return (int) get_post_meta( $post_id, $key, true );
}

function nictitate_lite_ii_toolkit_is_liked( $post_id = 0 ) {
    $key = sprintf( 'nictitate_like_by_%s', nictitate_lite_ii_toolkit_get_client_ip() );
    return (int) get_post_meta( $post_id, $key, true );
}

function nictitate_lite_ii_toolkit_single_share_post(){
