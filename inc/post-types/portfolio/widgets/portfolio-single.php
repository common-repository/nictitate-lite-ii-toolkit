...
add_action( 'kpb_get_widgets_list', array( 'Nictitate_Toolkit_II_Widget_Portfolio_Single', 'register_block' ) );
	
class Nictitate_Toolkit_II_Widget_Portfolio_Single extends Kopa_Widget {

	public $kpb_group = 'portfolio';
	
	public static function register_block( $blocks ) {
       	$blocks['Nictitate_Toolkit_II_Widget_Portfolio_Single'] = new Nictitate_Toolkit_II_Widget_Portfolio_Single();
        return $blocks;
    }
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display a single portfolio widget.', 'nictitate-lite-ii-toolkit' );
		$this->widget_id          = 'nictitate-toolkit-ii-widget-portfolio-single';
		$this->widget_name        = esc_html__( '__Single Portfolio', 'nictitate-lite-ii-toolkit' );
		$this->settings 		  = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => '',
			...
			'post_name' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Select a post', 'nictitate-lite-ii-toolkit' ),
				'std'     => '',
				'options' => $cbo_options
			),
			'excerpt_length'  => array(
				'type'  => 'text',
				'std'   => 20,
				'label' => esc_html__( 'Number of excerpt length', 'nictitate-lite-ii-toolkit' )
			)
		);
			...
			<?php if ( $result_set->have_posts() ): while( $result_set->have_posts() ) : $result_set->the_post(); ?>
				<div class="row item">
                    <div class="col-md-6 col-sm-6 col-xs-12 item-content">
						...
						?>
                        <a href="<?php the_permalink(); ?>" class="read-more read-more-border"><?php esc_html_e( 'view more', 'nictitate-lite-ii-toolkit' ); ?></a>
                    </div>
