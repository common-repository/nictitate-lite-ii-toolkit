...
function nictitate_toolkit_mailchimp_enqueue_scripts() {
    wp_enqueue_script( 'nictitate_toolkit_ii-mailchimp-script', NICTITATE_LITE_II_TOOLKIT_DIR . '/inc/post-types/contact/widgets/mailchimp-api/js/mailchimp.js', array( 'jquery' ), null );
    wp_localize_script('nictitate_toolkit_ii-mailchimp-script', 'nictitate_toolkit_ii_mailchimp', array(
        'ajaxurl' => admin_url( 'admin-ajax.php' ),
        'process' => esc_html__('Processing ...', 'nictitate-lite-ii-toolkit')
    ));
}
...
function nictitate_toolkit_add_subscriber_list() {
    if( !isset($_REQUEST['email']) || ! $_REQUEST['email'] ) {
        die( esc_html__( 'Please enter your email address.', 'nictitate-lite-ii-toolkit' ) );
    }

    if( !preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*$/i", $_REQUEST['email'])) {
        die( esc_html__( 'Email address is invalid.', 'nictitate-lite-ii-toolkit' ) );
    }
    ...
    if ( ! wp_verify_nonce( $nonce_d, 'nictitate_toolkit_mailchimp' ) ) {
        die( esc_html__( 'Security check', 'nictitate-lite-ii-toolkit' ) );
    }

    if( !isset( $_REQUEST['api_key'] ) || ! $_REQUEST['api_key'] ||
        !isset( $_REQUEST['list_id'] ) || ! $_REQUEST['list_id']
    ) {
        die( esc_html__( 'API Key or List ID is incorrect.', 'nictitate-lite-ii-toolkit' ) );
    }

    require_once( NICTITATE_LITE_II_TOOLKIT_PATH . '/inc/post-types/contact/widgets/mailchimp-api/inc/mailchimp-api-class.php');
    ...
    if( true === $api->listSubscribe($data_list_id, $data_email, '') ) {
        die( esc_html__( 'Success! Check your email to confirm sign up.', 'nictitate-lite-ii-toolkit' ) );
    }else{
