...
add_action( 'kpb_get_widgets_list', array( 'Nictitate_Toolkit_II_Widget_Posts_List_3_Col', 'register_block' ) );
	
class Nictitate_Toolkit_II_Widget_Posts_List_3_Col extends Kopa_Widget {

	public $kpb_group = 'post';
	
	public static function register_block( $blocks ) { 
       	$blocks['Nictitate_Toolkit_II_Widget_Posts_List_3_Col'] = new Nictitate_Toolkit_II_Widget_Posts_List_3_Col();
        return $blocks;
    }
    
	public function __construct() {
		$this->widget_cssclass    = 'k-widget-post-two-size-thumb';
		$this->widget_description = esc_html__( 'Display a posts list 3 col widget. First post big thumb.', 'nictitate-lite-ii-toolkit' );
		$this->widget_id          = 'nictitate-toolkit-ii-widget-posts-list-3-col';
		$this->widget_name        = esc_html__( '__Posts List 3 Col', 'nictitate-lite-ii-toolkit' );
		$this->settings 		  = nictitate_lite_ii_toolkit_get_post_widget_args();
		$this->settings['excerpt_length_big_thumb'] = array(
			'type'  => 'number',
			'std'   => 20,
			'label' => esc_html__( 'Excerpt length post big thumb:', 'nictitate-lite-ii-toolkit' ),
			'desc'  => ''
		);
		...
		$this->settings['button_text'] = array(
			'type'  => 'text',
			'std'   => esc_html__('view all post', 'nictitate-lite-ii-toolkit'),
	...
	public function widget( $args, $instance ) {
		...
		$query      = nictitate_lite_ii_toolkit_get_post_widget_query( $instance );
										...
										<div class="item-thumb">
											...
											<div class="item-metadata k-item-metadata">
												...
												<span class="item-metadata-comments">
													...
													<span class="item-metadata-comments-title"><?php esc_html_e( 'Com', 'nictitate-lite-ii-toolkit' ); ?></span>
												</span>
											</div>
										</div>
										<h4 class="item-title"><a href="<?php the_permalink();?>"><?php the_title(); ?></a></h4>
										<?php 
											$GLOBALS['nictitate_lite_ii_excerpt_length'] = (int) $excerpt_length_big_thumb;
