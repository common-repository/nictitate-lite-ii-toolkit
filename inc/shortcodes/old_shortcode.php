<?php
add_shortcode('contact_form', 'nictitate_toolkit_ii_old_shortcode_contactform');
function nictitate_toolkit_ii_old_shortcode_contactform($atts, $content = null){
	return do_shortcode('[nictitate_toolkit_ii_contactform]');
}
...
function kopa_shortcode_toggles($atts = array(), $content = NULL) {
	...
	$tab_classes = apply_filters('nictitate_toolkit_ii_d001_shortcode_accordions_classes', 'style-1', $style_id );
	...
	$matches = nictitate_lite_ii_toolkit_extract_shortcodes( $content, true, array( 'toggle' ) );
	...
	return apply_filters( 'nictitate_toolkit_ii_d001_accordions', $string, $atts, $content );
...
function kopa_shortcode_posts($atts, $content = null) {
    ...
    switch ($atts['orderby']) {
        case 'popular':
            $args['meta_key'] = 'kopa_' . 'nictitate-lite-ii-toolkit' . '_total_view';
    ...
    if ($posts->post_count > 0) {
        ...
        while ($posts->have_posts()) {
            ...
            if (comments_open()) {
                if ($num_comments == 0) {
                    $comments = esc_html__('0 Comments', 'nictitate-lite-ii-toolkit');
                } elseif ($num_comments > 1) {
                    $comments = $num_comments . esc_html__(' Comments', 'nictitate-lite-ii-toolkit');
                } else {
                    $comments = esc_html__('1 Comment', 'nictitate-lite-ii-toolkit');
                }
                $out .= $comments;
            } else {
                $out .= esc_html__('Comments off', 'nictitate-lite-ii-toolkit');
            }
            $out .= '</span>';
            $out .= '</div>';
            $out .= '</article>';
