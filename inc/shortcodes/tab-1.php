<?php
add_filter('nictitate_lite_ii_toolkit_get_elements', 'nictitate_toolkit_ii_register_elements_tabs');


function nictitate_toolkit_ii_register_elements_tabs($groups){
	$groups['tabs'][] = array(
		'name' => esc_html__('Tab 1','nictitate-lite-ii-toolkit'),
...
add_shortcode( 'tabs', 'nictitate_toolkit_ii_shortcode_tabs' );
add_shortcode( 'tab', '__return_false' );

function nictitate_toolkit_ii_shortcode_tabs( $atts, $content ) {
	$rand = rand();
	...
	$tab_classes = apply_filters('nictitate_toolkit_ii_shortcode_tabs_classes', 'style-1', $style_id );

	ob_start();
	$matches = nictitate_lite_ii_toolkit_extract_shortcodes( $content, true, array( 'tab' ) );
	...
	ob_end_clean();
	return apply_filters( 'nictitate_toolkit_ii_shortcode_tabs', $string, $atts, $content );
}
