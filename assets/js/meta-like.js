...
jQuery(document).ready(function($) {
    var buttons = jQuery('.nictitate_lite_ii_toolkit_like_clickable');    
    if(buttons.length){
...
var nictitate_toolkit_ii_like = {
    click_like_button: function(event, obj, post_id) {
        ...
        if (!obj.hasClass('inprocess')) {
            ...
            jQuery.ajax({
                type: 'POST',
                url: nictitate_lite_ii_toolkit_meta_likes.ajax.url.meta_like,
                dataType: 'json',
                async: true,
                timeout: 5000,
                data: {
                    action: 'nictitate_lite_ii_toolkit_meta_like',                    
                    post_id: post_id,                    
                    status: status
                },
